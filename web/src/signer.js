...
export function getCertificateChainDer() {
    return pemsToDer(DEV_CERT_PEM);
}
export function getCertificateChainPem() {
    return DEV_CERT_PEM;
}
...
export async function signRaw(data) {
    const key = await getPrivateKey();
    const sig = await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, key, data.buffer);
    return new Uint8Array(sig);
}
export async function signCose(dataToBeSigned, reserveSize) {
    ...
    const key = await getPrivateKey();
    const signature = new Uint8Array(await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, key, sigStructure.buffer));
    const rawSig = normalizeEcdsaSignature(signature);
    console.log("[sign-ai-media] raw ECDSA signature size:", rawSig.length);
    return buildCoseSign1(protectedHeaders, rawSig, reserveSize);
}
