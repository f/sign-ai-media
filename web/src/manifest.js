...
export const DEFAULT_DIGITAL_SOURCE_TYPE = `${IPTC_BASE}trainedAlgorithmicMedia`;
export function createAiGeneratedManifest(opts) {
    ...
    const softwareAgent = { name: metadata.softwareAgent };
    if (metadata.version)
        softwareAgent.version = metadata.version;
    const digitalSourceType = metadata.digitalSourceType ?? DEFAULT_DIGITAL_SOURCE_TYPE;
    const claimGenerator = metadata.claimGenerator ?? formatClaimGenerator(metadata.softwareAgent, metadata.version);
        ...
        assertions: [
                ...
                : []),
            {
                ...
                data: {
                    ...
                    "@type": "CreativeWork",
                    generator: metadata.generator ?? metadata.model ?? metadata.softwareAgent,
...
function formatClaimGenerator(name, version) {
