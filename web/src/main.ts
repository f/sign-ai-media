...
import type { AiMediaMetadata } from "./manifest";
...
async function prefillFromFile(file: File) {
  clearFormFields();
  ...
  try {
    ...
    if (dst) {
      ...
      if (preset) {
        ($("source-type") as HTMLSelectElement).value = preset[0];
      }
    }
  } catch {
    setField("software-agent", file.name.replace(/\.[^.]+$/, ""));
...
function clearFormFields() {
  ...
  ($("source-type") as HTMLSelectElement).value = "ai-generated";
}
...
function setupSourceTypeSelect() {
  const select = $("source-type") as HTMLSelectElement;
  for (const key of Object.keys(DIGITAL_SOURCE_TYPE_PRESETS)) {
    const opt = document.createElement("option");
    opt.value = key;
    opt.textContent = key;
    if (key === "ai-generated") opt.selected = true;
...
async function doView(file: File) {
  const out = $("output");
...
async function doSign(file: File) {
  ...
  const metadata: AiMediaMetadata = {
    ...
    digitalSourceType:
      DIGITAL_SOURCE_TYPE_PRESETS[get("source-type")] || undefined,
    seed: get("seed") || undefined,
  };

  if (!file.type && !inferMimeType(file.name)) {
    showError(
      "Cannot detect MIME type. Use a file with a known extension (png, jpg, webp, mp4, etc.).",
    );
    return;
  }
